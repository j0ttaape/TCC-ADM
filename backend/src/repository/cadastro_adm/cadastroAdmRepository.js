...
const comando = `
insert into cadastro_adm(nome,email, senha, permissao,permissao_adm )
values
(?,?,MD5(?),false,false);
`
...
export async function listarPedidosAdm() {
    const comando = `
    select * from cadastro_adm
    where permissao_adm = false and permissao = true
    order by nome
    `
...
export async function concederPermissaoAdm(id_requerido,id_adm){
    const comando = `
    select permissao_adm from cadastro_adm
    where id_adm = ?
    `
    const [adm] = await connection.query(comando,[id_adm]);
    ...
    const perm = adm[0].permissao_adm;
    ...
    const assunto = `Seu acesso de administrador foi aprovado 🎉`;
    const texto = `Olá, ${nome[0].nome}!

Temos o prazer de informar que seu pedido para ingressar como administrador do site foi aprovado.

Agora você possui acesso completo às ferramentas e recursos administrativos.
Por favor, utilize suas credenciais para entrar no painel e começar a colaborar com a equipe.

Seja bem-vindo(a) como administrador! 👏
...
await transporter.sendMail({
    ...
    text: texto
        ...
        let resposta = 'Usuário Permitido como Administrador'
        return resposta
    }
    else {
        let resposta = 'Você não tem permissão para essa ação'
